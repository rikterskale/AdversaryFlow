...
function reportSummary(s){if(s.status!=='available')return '<p class="muted">'+esc(s.detail)+'</p>';let gaps=s.gaps.length?s.gaps.map(g=>'<li>'+esc(g.technique_id||g.category||'unknown')+': '+esc(g.status||g.description||'missing')+'</li>').join(''):'<li>No required telemetry gaps reported.</li>';return '<p><strong>Behavior:</strong> '+(s.behavior_success?'completed':'needs review')+'<br><strong>Telemetry:</strong> '+esc(s.telemetry_observed)+' observed of '+esc(s.telemetry_expected)+' expected<br><strong>Detection gaps:</strong> '+esc(s.detection_gap_count)+'</p><p class="muted">'+esc(s.assessment)+'</p><details><summary>Detection-gap findings</summary><ul>'+gaps+'</ul></details>'}

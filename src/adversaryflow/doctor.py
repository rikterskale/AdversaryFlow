...
def run_doctor(roe_path: str = "examples/roe.yaml", catalog_path: str = "content/abilities/catalog.json") -> dict[str, Any]:
    ...
    return {"product": "AdversaryFlow", "version": "0.1.0", "platform": detected, "platform_detail": platform.platform(), "cwd": os.getcwd(), "supported_platforms": ["Windows", "Debian", "Ubuntu", "Kali"], "passed": all(bool(item["passed"]) for item in checks), "checks": checks}

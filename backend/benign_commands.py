"""
Benign command library for AdversaryFlow.

For each ATT&CK technique we provide *benign* proxy commands: harmless,
non-destructive actions that exercise the same tools / APIs / artifacts a real
attacker would, so detections fire without any damage being done. This mirrors
the philosophy of Atomic Red Team detection tests, restricted to the safe
subset (discovery, LOLBin invocation, benign writes to temp paths).

CONTRACT for every command here:
  * No data destruction, no reboot-surviving persistence, no privilege changes,
    no callbacks to real infrastructure.
  * Anything that writes creates a clearly-labelled temp artifact.
  * Commands that leave an artifact include a `cleanup`.

`get_commands()` returns curated commands when available and otherwise a
tactic-aware safe fallback, so *every* technique in a workflow is runnable.
"""
...
# ---------------------------------------------------------------------------
# Curated library: ATT&CK technique id -> list of benign proxy commands.
# ---------------------------------------------------------------------------

CURATED: Dict[str, List[Dict[str, str]]] = {
    # ---- Execution ------------------------------------------------------
    "T1059.001": [_c("windows", "powershell.exe -NoProfile -Command \"Write-Host 'AdversaryFlow benign PowerShell exec test'\"",
                     "Inline PowerShell to exercise script-block / EDR logging.")],
    "T1059.003": [_c("windows", "cmd.exe /c \"echo AdversaryFlow benign cmd exec test\"",
                     "Benign cmd.exe child process for command-line logging.")],
    "T1059.004": [_c("linux", "/bin/bash -c \"echo AdversaryFlow benign shell exec test\"",
                     "Benign bash invocation for shell-execution telemetry."),
                  _c("macos", "/bin/bash -c \"echo AdversaryFlow benign shell exec test\"")],
    "T1059.005": [_c("windows", "cscript.exe //B //Nologo //E:vbscript nul 2>nul & echo AdversaryFlow benign WSH host test",
                     "Invokes the Windows Script Host binary without a malicious script.")],
    "T1059.006": [_c("windows", "python -c \"print('AdversaryFlow benign python exec test')\"",
                     "Runs the python interpreter with a harmless one-liner."),
                  _c("linux", "python3 -c \"print('AdversaryFlow benign python exec test')\"")],
    "T1059.007": [_c("windows", "mshta.exe about:blank & echo AdversaryFlow benign scripting-host proxy",
                     "Launches mshta against a blank page (no remote script).",
                     "taskkill /IM mshta.exe /F 2>nul")],
    "T1053.005": [_c("windows", "schtasks /Create /TN AdversaryFlowBenign /TR \"cmd.exe /c echo hi\" /SC ONCE /ST 23:59 /F",
                     "Creates a benign one-time scheduled task to trip task-creation detections.",
                     "schtasks /Delete /TN AdversaryFlowBenign /F")],
    "T1053.003": [_c("linux", "( crontab -l 2>/dev/null; echo '# AdversaryFlow benign cron marker' ) | crontab -",
                     "Adds a benign comment line to the user crontab.",
                     "crontab -l | grep -v 'AdversaryFlow benign cron marker' | crontab -")],
    "T1204.002": [_c("windows", "cmd.exe /c \"echo AdversaryFlow benign 'user opened attachment' proxy\"",
    ...
    "T1569.002": [_c("windows", "sc.exe query wuauserv", "Benign service query (proxy for service execution).")],
    "T1203": [_c("windows", "cmd.exe /c \"echo AdversaryFlow benign exploitation-for-execution proxy (no exploit run)\"",
                 "Placeholder only — no real exploit is ever executed.")],
    "T1129": [_c("windows", "powershell -NoProfile -Command \"[Reflection.Assembly]::LoadWithPartialName('System.Xml') | Out-Null; 'benign module load'\"",
                 "Loads a benign assembly to exercise image-load telemetry.")],
    ...
    "T1046": [_c("windows", "powershell -NoProfile -Command \"Test-NetConnection -ComputerName localhost -Port 445 | Select RemoteAddress,TcpTestSucceeded\"",
                 "Single benign local port check (not a scan).")],
    ...
    "T1497.001": [_c("windows", "wmic computersystem get model,manufacturer", "Benign virtualization/sandbox check (read-only).")],
    "T1614": [_c("windows", "wmic os get locale,countrycode,oslanguage /format:list", "System location discovery.")],

    # ---- Credential Access ---------------------------------------------
    "T1003.001": [_c("windows", "cmd.exe /c \"echo AdversaryFlow benign LSASS-access proxy && tasklist | findstr lsass\"",
                     "Locates lsass without dumping it — no memory is read.")],
    "T1003.002": [_c("windows", "reg save HKLM\\SAM %TEMP%\\af_sam_test.hiv 2>nul & echo (requires admin; benign copy)",
                     "Benign SAM export to a temp file to trip credential-store access alerts.",
    ...
    "T1110": [_c("windows", "cmd.exe /c \"echo AdversaryFlow benign brute-force proxy - no auth attempts made\"",
                 "Placeholder only — no authentication attempts are generated.")],
    "T1056.001": [_c("windows", "cmd.exe /c \"echo AdversaryFlow benign keylogging proxy - no hooks installed\"",
                     "Placeholder only — no input hooks are installed.")],

    # ---- Persistence ----------------------------------------------------
    "T1547.001": [_c("windows", "reg add HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run /v AdversaryFlowBenign /t REG_SZ /d \"cmd.exe /c echo hi\" /f",
                     "Benign Run-key write to trip autostart detections.",
                     "reg delete HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run /v AdversaryFlowBenign /f")],
    "T1543.003": [_c("windows", "sc.exe create AdversaryFlowBenign binPath= \"cmd.exe /c echo hi\" start= demand & echo (requires admin)",
                     "Creates a benign, non-running service.",
                     "sc.exe delete AdversaryFlowBenign")],
    ...
    "T1136.001": [_c("windows", "net user AdversaryFlowTmp P@ssw0rd!23 /add & echo (requires admin)",
                     "Creates a benign local account to trip account-creation alerts.",
                     "net user AdversaryFlowTmp /delete")],
    "T1098": [_c("windows", "net user %USERNAME% & echo AdversaryFlow benign account-manipulation proxy (read-only)",
                 "Read-only account inspection as an account-manipulation proxy.")],
    "T1574.002": [_c("windows", "cmd.exe /c \"echo AdversaryFlow benign DLL-side-load proxy - no DLL planted\"",
                     "Placeholder only — no DLL is planted.")],
    "T1505.003": [_c("windows", "cmd.exe /c \"echo AdversaryFlow benign web-shell proxy - no file written to webroot\"",
    ...
    "T1055": [_c("windows", "powershell -NoProfile -Command \"[Diagnostics.Process]::Start('notepad.exe'); Start-Sleep 1; Stop-Process -Name notepad\"",
                 "Spawns/kills notepad as a benign process-interaction proxy — no injection.",
                 "taskkill /IM notepad.exe /F 2>nul")],

    # ---- Defense Evasion -----------------------------------------------
    "T1070.001": [_c("windows", "wevtutil qe Security /c:1 /rd:true /f:text 2>nul", "Reads (not clears) an event-log entry as a benign proxy.")],
    "T1070.004": [_c("windows", "echo AdversaryFlow benign > %TEMP%\\af_marker.txt & del %TEMP%\\af_marker.txt",
                     "Creates and deletes a benign temp file (file-deletion telemetry).")],
    "T1112": [_c("windows", "reg add HKCU\\Software\\AdversaryFlowBenign /v test /t REG_SZ /d benign /f",
                 "Benign registry modification under a scratch key.",
                 "reg delete HKCU\\Software\\AdversaryFlowBenign /f")],
    "T1027": [_c("windows", "powershell -NoProfile -Command \"[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes('AdversaryFlow benign'))\"",
                 "Base64-encodes a benign string (obfuscation proxy).")],
    "T1140": [_c("windows", "powershell -NoProfile -Command \"[Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('QWR2ZXJzYXJ5Rmxvdw=='))\"",
                 "Decodes a benign base64 string (deobfuscation proxy).")],
    "T1218.005": [_c("windows", "mshta.exe about:blank & echo AdversaryFlow benign mshta LOLBin proxy",
                     "Runs the mshta LOLBin against a blank page.", "taskkill /IM mshta.exe /F 2>nul")],
    "T1218.011": [_c("windows", "rundll32.exe user32.dll,LockWorkStation & echo (locks screen - benign LOLBin)",
                     "Uses rundll32 to call a benign, documented export.")],
    "T1197": [_c("windows", "bitsadmin /list & echo AdversaryFlow benign BITS proxy", "Lists BITS jobs (read-only LOLBin use).")],
    "T1562.001": [_c("windows", "sc.exe query windefend", "Read-only query of the defender service (does NOT disable it).")],
    "T1036": [_c("windows", "copy %WINDIR%\\System32\\calc.exe %TEMP%\\svch0st.exe & echo (benign rename)",
                 "Copies calc.exe under a masquerading name in temp.", "del %TEMP%\\svch0st.exe 2>nul")],
    "T1497": [_c("windows", "wmic bios get serialnumber & wmic computersystem get model", "Benign sandbox/VM checks (read-only).")],
    ...
    # ---- Lateral Movement ----------------------------------------------
    "T1021.001": [_c("windows", "cmdkey /list & echo AdversaryFlow benign RDP-proxy (no session opened)",
                     "Enumerates stored RDP creds without opening a session.")],
    "T1021.002": [_c("windows", "net use \\\\localhost\\IPC$ & echo (benign IPC connection)",
                     "Opens a benign loopback SMB/IPC session.", "net use \\\\localhost\\IPC$ /delete 2>nul")],
    "T1021.006": [_c("windows", "powershell -NoProfile -Command \"Test-WSMan -ComputerName localhost | Select ProductVendor\"",
                     "Benign WinRM check against localhost.")],
    "T1570": [_c("windows", "copy %WINDIR%\\System32\\calc.exe %TEMP%\\af_lateral.exe & echo (benign local copy)",
    ...
    "T1560.001": [_c("windows", "powershell -NoProfile -Command \"Compress-Archive -Path $env:WINDIR\\win.ini -DestinationPath $env:TEMP\\af_collect.zip -Force\"",
                     "Archives a benign file as a collection/staging proxy.", "del %TEMP%\\af_collect.zip 2>nul"),
    ...
    "T1039": [_c("windows", "net view \\\\localhost & echo AdversaryFlow benign network-share-collection proxy",
    ...
    "T1071.001": [_c("windows", "powershell -NoProfile -Command \"Invoke-WebRequest https://example.com -UseBasicParsing | Select StatusCode\"",
                     "Benign HTTPS request to a safe host (app-layer C2 proxy)."),
    ...
    "T1105": [_c("windows", "powershell -NoProfile -Command \"Invoke-WebRequest https://example.com/index.html -OutFile $env:TEMP\\af_download.html\"",
                 "Benign ingress tool-transfer proxy to a temp file.", "del %TEMP%\\af_download.html 2>nul"),
    ...
    "T1571": [_c("windows", "powershell -NoProfile -Command \"Test-NetConnection example.com -Port 8080 | Select TcpTestSucceeded\"",
                 "Single benign connection attempt to a non-standard port.")],
    "T1573": [_c("windows", "powershell -NoProfile -Command \"Invoke-WebRequest https://example.com -UseBasicParsing | Select StatusCode\"",
                 "Encrypted-channel proxy (ordinary TLS request to a safe host).")],
    "T1090": [_c("windows", "netsh interface portproxy show all & echo AdversaryFlow benign proxy-config read",
                 "Read-only view of port-proxy configuration.")],
    "T1568": [_c("windows", "nslookup example.com", "Benign DNS resolution (dynamic-resolution proxy).")],

    # ---- Exfiltration ---------------------------------------------------
    "T1041": [_c("windows", "powershell -NoProfile -Command \"$b=[Text.Encoding]::UTF8.GetBytes('benign'); (Invoke-WebRequest -Uri https://example.com -Method POST -Body $b -UseBasicParsing).StatusCode\"",
                 "Posts a tiny benign body to a safe host (C2-channel exfil proxy).")],
    ...
    "T1048": [_c("windows", "nslookup example.com & echo AdversaryFlow benign alt-protocol-exfil proxy",
                 "Benign DNS lookup as an alternative-protocol exfil proxy.")],
    "T1030": [_c("windows", "cmd.exe /c \"echo AdversaryFlow benign data-transfer-size-limits proxy\"",
                 "Placeholder only — no data is transferred.")],

    # ---- Impact ---------------------------------------------------------
    "T1486": [_c("windows", "echo AdversaryFlow benign > %TEMP%\\af_ransim.txt & type %TEMP%\\af_ransim.txt & del %TEMP%\\af_ransim.txt",
                 "Writes/reads/deletes ONE temp file — no real encryption occurs.")],
    "T1490": [_c("windows", "vssadmin list shadows & echo AdversaryFlow benign proxy (does NOT delete shadows)",
                 "Lists shadow copies read-only — never deletes them.")],
    "T1489": [_c("windows", "sc.exe query wuauserv & echo AdversaryFlow benign service-stop proxy (read-only)",
                 "Queries a service without stopping it.")],
    "T1529": [_c("windows", "cmd.exe /c \"echo AdversaryFlow benign shutdown proxy - no shutdown issued\"",
                 "Placeholder only — no shutdown/reboot is issued.")],
    "T1491.001": [_c("windows", "cmd.exe /c \"echo AdversaryFlow benign defacement proxy - nothing changed\"",
                     "Placeholder only.")],

    # ---- Initial Access -------------------------------------------------
    "T1566.001": [_c("windows", "cmd.exe /c \"echo AdversaryFlow benign spearphishing-attachment proxy - no email/file involved\"",
    ...
    "T1566.002": [_c("windows", "powershell -NoProfile -Command \"Invoke-WebRequest https://example.com -UseBasicParsing | Select StatusCode\"",
                     "Benign HTTPS fetch as a phishing-link click proxy.")],
    "T1078": [_c("windows", "whoami /all & echo AdversaryFlow benign valid-accounts proxy (read-only)",
                 "Read-only identity inspection — no credentials used.")],
    "T1190": [_c("windows", "cmd.exe /c \"echo AdversaryFlow benign exploit-public-app proxy - no exploit run\"",
    ...
    "T1189": [_c("windows", "powershell -NoProfile -Command \"Invoke-WebRequest https://example.com -UseBasicParsing | Select StatusCode\"",
                 "Benign web fetch as a drive-by-compromise proxy.")],
}
...
TACTIC_FALLBACK: Dict[str, Dict[str, str]] = {
    "reconnaissance": _c("windows", "nslookup {domain} & echo AdversaryFlow benign recon proxy for {tid}",
                         "Benign DNS lookup as a reconnaissance proxy."),
    "resource-development": _c("windows", "cmd.exe /c \"echo AdversaryFlow benign resource-development proxy for {tid} - nothing provisioned\"",
                              "Placeholder — no infrastructure/tooling is provisioned."),
    "initial-access": _c("windows", "cmd.exe /c \"echo AdversaryFlow benign initial-access proxy for {tid} - no delivery performed\"",
                        "Placeholder for the delivery step."),
    "execution": _c("windows", "cmd.exe /c \"echo AdversaryFlow benign execution proxy for {tid}\"",
                   "Spawns a benign child process to exercise execution telemetry."),
    "persistence": _c("windows", "reg query HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run & echo AdversaryFlow benign persistence-inspection for {tid}",
                     "Read-only inspection of a common persistence location."),
    "privilege-escalation": _c("windows", "whoami /priv & echo AdversaryFlow benign privesc proxy for {tid}",
    ...
    "defense-evasion": _c("windows", "powershell -NoProfile -Command \"[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes('AdversaryFlow {tid}'))\"",
                         "Benign obfuscation proxy (base64 of a marker string)."),
    "stealth": _c("windows", "powershell -NoProfile -Command \"[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes('AdversaryFlow {tid}'))\"",
                 "Benign obfuscation proxy (base64 of a marker string)."),
    "defense-impairment": _c("windows", "sc.exe query windefend & echo AdversaryFlow benign defense-impairment proxy for {tid} (read-only)",
                            "Read-only query of a security service (nothing disabled)."),
    "credential-access": _c("windows", "cmdkey /list & echo AdversaryFlow benign credential-access proxy for {tid}",
                           "Read-only enumeration of stored credentials."),
    "discovery": _c("windows", "systeminfo & echo AdversaryFlow benign discovery proxy for {tid}",
                   "Benign read-only host discovery."),
    "lateral-movement": _c("windows", "net view /all & echo AdversaryFlow benign lateral-movement proxy for {tid}",
                          "Read-only remote-system enumeration (no session opened)."),
    "collection": _c("windows", "dir /s /b %USERPROFILE%\\*.docx 2>nul | more & echo AdversaryFlow benign collection proxy for {tid}",
    ...
    "command-and-control": _c("windows", "powershell -NoProfile -Command \"Invoke-WebRequest https://example.com -UseBasicParsing | Select StatusCode\"",
                             "Benign HTTPS request to a safe host as a C2 proxy."),
    "exfiltration": _c("windows", "nslookup example.com & echo AdversaryFlow benign exfiltration proxy for {tid} - no data sent",
                      "Benign DNS lookup; no data leaves the host."),
    "impact": _c("windows", "cmd.exe /c \"echo AdversaryFlow benign impact proxy for {tid} - system unchanged\"",
                "Placeholder — no destructive action is taken."),
}

GENERIC_FALLBACK = _c("windows", "cmd.exe /c \"echo AdversaryFlow benign proxy for {tid} ({name})\"",
                      "Generic benign marker — review the technique and craft a safe test.")
...
try:
    from benign_commands_extended import EXTENDED as _EXTENDED
...
def get_commands(technique_id: str, technique_name: str, tactics: List[str],
    ...
    """Return benign commands for a technique.

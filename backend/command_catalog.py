...
CURATED: Dict[str, List[Dict[str, Any]]] = {
    ...
    "T1059.006": [_c("windows", "python -c \"print('AdversaryFlow lab python exec test')\"",
                  ...
                  _c("linux", "python3 -c \"print('AdversaryFlow lab python exec test')\"",
                     "Runs the Python interpreter with a lab marker one-liner."),
                  _c("macos", "python3 -c \"print('AdversaryFlow lab python exec test')\"",
                     "Runs the Python interpreter with a lab marker one-liner.")],
    ...
    "T1053.005": [_c("windows", "schtasks /Create /TN AdversaryFlowLab /TR \"cmd.exe /c echo hi\" /SC ONCE /ST 23:59 /F",
                     ...
                     risk="high", side_effects=["changes_local_state"], acknowledgment_required=True,
                     prerequisites=["windows command environment", "authorized disposable lab", "operator approval to create a scheduled task"])],
    "T1053.003": [_c("linux", "( crontab -l 2>/dev/null; echo '# AdversaryFlow lab cron marker' ) | crontab -",
                     ...
                     risk="high", side_effects=["changes_local_state"], acknowledgment_required=True,
                     prerequisites=["linux command environment", "authorized disposable lab", "operator approval to edit crontab"])],
    ...
    "T1003.002": [_c("windows", "if exist %WINDIR%\\System32\\config\\SAM (echo AdversaryFlow lab: SAM hive present) else (echo AdversaryFlow lab: SAM hive not present)",
                     "Locates the SAM hive without exporting or reading it.")],
    "T1552.001": [_c("windows", "cmd.exe /c \"echo password=AdversaryFlowLabMarker> %TEMP%\\af_cred_search.txt & findstr /i password %TEMP%\\af_cred_search.txt & del %TEMP%\\af_cred_search.txt\"",
                     "Searches a lab temp file for a planted marker — does not scan the user profile.",
                     "del %TEMP%\\af_cred_search.txt 2>nul"),
                  _c("linux", "printf 'password=AdversaryFlowLabMarker\\n' > /tmp/af_cred_search.txt && grep -i password /tmp/af_cred_search.txt; rm -f /tmp/af_cred_search.txt",
                     "Searches a lab temp file for a planted marker — does not scan /etc.")],
    ...
    "T1547.001": [_c("windows", "reg add HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run /v AdversaryFlowLab /t REG_SZ /d \"cmd.exe /c echo hi\" /f",
                     ...
                     risk="medium", side_effects=["changes_local_state"], acknowledgment_required=True,
                     prerequisites=["windows command environment", "authorized disposable lab", "operator approval to write a current-user Run key"])],
    "T1543.003": [_c("windows", "sc.exe query type= service state= all | more",
                     "Lists services (service-creation proxy). Does not create or modify a service.")],
    ...
    "T1136.001": [_c("windows", "net user",
                     "Lists local accounts (account-creation proxy). Does not create an account.")],
    ...
    "T1547.005": [_c("windows", "reg query HKLM\\SYSTEM\\CurrentControlSet\\Control\\Lsa /v SecurityPackages",
                     "Read-only SSP package list. Does not install a security support provider.")],
    "T1547.008": [_c("windows", "sc.exe query type= driver | more",
                     "Read-only driver inventory as an LSASS-driver persistence proxy.")],
    "T1547.014": [_c("windows", "reg query \"HKLM\\SOFTWARE\\Microsoft\\Active Setup\\Installed Components\"",
                     "Read-only Active Setup component list. Does not install a component.")],
    "T1070.009": [_c("windows", "cmd.exe /c \"echo AdversaryFlow lab clear-persistence proxy - nothing deleted\"",
                     "Lab proxy — no persistence artifacts are removed.")],
    "T1207": [_c("windows", "nltest /dclist: 2>nul & echo AdversaryFlow lab rogue-DC proxy (read-only)",
                 "Read-only DC list. Does not promote a domain controller.")],
    "T1134.005": [_c("windows", "whoami /all & echo AdversaryFlow lab SID-History proxy (read-only)",
                     "Inspects the current token. Does not inject SID history.")],
    "T1558.002": [_c("windows", "cmd.exe /c \"echo AdversaryFlow lab silver-ticket proxy - no ticket forged\"",
                     "Lab proxy — no Kerberos ticket is requested or forged.")],
    ...
    "T1218.011": [_c("windows", "where rundll32.exe & echo AdversaryFlow lab rundll32 locate — screen is not locked",
    ...
    "T1105": [_c("windows", "powershell -NoProfile -Command \"Set-Content -Path $env:TEMP\\af_download.html -Value 'AdversaryFlow lab transfer marker'\"",
              ...
              _c("linux", "printf 'AdversaryFlow lab transfer marker\\n' > /tmp/af_download.html && rm -f /tmp/af_download.html",
                 "Writes a local transfer marker. Does not download from the network.")],
    ...
    "T1048": [_c("windows", "nslookup localhost & echo AdversaryFlow lab alt-protocol-exfil proxy",
                 "Loopback DNS lookup as an alternative-protocol exfil proxy.")],
...
TACTIC_FALLBACK: Dict[str, Dict[str, str]] = {
    "reconnaissance": _c("windows", "nslookup localhost & echo AdversaryFlow lab recon proxy for {tid}",
                         "Loopback DNS lookup as a reconnaissance proxy."),

...
export function TechniqueCard({ technique, evidence, focused, firstLab, onFocus, onUpdate, onCopy, onNotice }: TechniqueCardProps): JSX.Element {
  ...
  return (
    <article className={`techcard ${marked ? "is-run" : ""} ${focused ? "is-focused" : ""} ${firstLab ? "is-firstlab" : ""}`} data-tid={technique.attack_id} onClick={onFocus} tabIndex={-1}>
      ...
      <div className={`preaction ${unsupported ? "preaction--unsupported" : ""}`}>
        <div className="preaction__head"><span>What this will touch</span><strong>Review before copy</strong></div>
        <dl className="preaction__grid">
          <div><dt>Risk</dt><dd><span className={`riskvalue riskvalue--${command.risk}`}>{titleValue(command.risk)}</span></dd></div>
          ...
          <div><dt>Rollback</dt><dd>{command.rollback || command.cleanup || "No rollback action specified"}</dd></div>
        </dl>
        {command.side_effects.length ? <p><strong>Side effects</strong>{command.side_effects.map(titleValue).join(" · ")}</p> : null}
      </div>
      ...
      {!unsupported ? (
        <div className={`safety safety--${command.risk}`}>
          ...
          <div className="safety__grid">
            <div><strong>Effects:</strong> {command.side_effects.length ? command.side_effects.map(titleValue).join(", ") : "Not classified"} · <strong>Expected:</strong> {command.expected_telemetry || "Verify relevant telemetry"}</div>
      ...
      {!unsupported ? (
        <details className="evidenceproof" onClick={(event) => event.stopPropagation()}>
          ...
          <div className="evidenceproof__grid">
            ...
            <label>Started (ISO 8601)<input onChange={(event) => onUpdate({ started_at: event.target.value || undefined })} value={evidence?.started_at ?? ""} /></label>
            <label>Completed (ISO 8601)<input onChange={(event) => onUpdate({ completed_at: event.target.value || undefined })} value={evidence?.completed_at ?? ""} /></label>

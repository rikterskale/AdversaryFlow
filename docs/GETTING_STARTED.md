...
A copy/paste-safe operator handbook for **AdversaryFlow 0.5.2**.
...
- [ ] `adversaryflow --version` prints `AdversaryFlow 0.5.2`
...
| Browser never opens, nothing listens | Start failed, or you closed the terminal | Run `./run.sh` again; confirm `AdversaryFlow 0.5.2: http://127.0.0.1:5000` |
...
AdversaryFlow **0.5.2** is the current package version
...
```bash
pipx install ./adversaryflow-0.5.2-py3-none-any.whl

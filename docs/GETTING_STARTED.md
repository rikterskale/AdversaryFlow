...
A copy/paste-safe operator handbook for **AdversaryFlow 0.5.0**.
...
AdversaryFlow 0.5.0: http://127.0.0.1:5000
```
...
- [ ] `adversaryflow --version` prints `AdversaryFlow 0.5.0`
...
| Browser never opens, nothing listens | Start failed, or you closed the terminal | Run `./run.sh` again; confirm `AdversaryFlow 0.5.0: http://127.0.0.1:5000` |
...
AdversaryFlow **0.5.0** is the current package version
...
Verified against `adversaryflow --help` on 0.5.0. Default command is
`serve`.

...
reports, and schema-versioned planning records. Every server-generated
artifact is serialized only after the submitted plan is rebound to the live
bounded catalog. The web service never executes a command.
...
**PDF engagement report** and **HTML engagement report** are generated by the
CSRF-protected `POST /api/report/{format}` endpoint. Both formats contain:

- planned and unique technique counts;
...
content-security policy. The PDF is dependency-free at runtime, paginated,
and includes the plan digest and ATT&CK data version for provenance. Neither
format claims a community Sigma rule does not exist when the catalog has no
reference.

JSON remains the canonical machine-readable plan and evidence record. The
HTML/PDF reports are human-facing views derived from that record, not new
schema versions.

...
installation, `adversaryflow --version` prints `AdversaryFlow 0.5.0`.
...
|---|---|---|
| `adversaryflow --version` | Identify the build | `AdversaryFlow 0.5.0` |
...
| J2 | Verify install | `adversaryflow --version` | Prints the packaged version | stdout is exactly `AdversaryFlow 0.5.0` |
...
| J4 | Launch | `./run.sh` | Starts waitress on loopback and opens a browser when ready | stdout contains `AdversaryFlow 0.5.0: http://127.0.0.1:5000` |
